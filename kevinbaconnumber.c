...
struct movienode* mq_enqueue(struct movie* x){	//moviequeue_enqueue parametre olarak aldigi bir movie yi queueya ekler ve movienode tipinde sonuc dondurur.
	...
	newNode->data=x;	//filmi newNode un datasına ekledik
	...
	if(firstm==NULL){	//ilk eleman icin  (queuenin first u null yani eklenen,ilk eleman ise)
		...
		lastm->next=newNode;	//lastm ve lastm in nextini newNode yaptık
		lastm=newNode;
	}
...
struct actornode* aq_enqueue(struct actor* x){	//actorqueue_enqueue parametre olarak aldigi bir aktoru queueya ekler ve actornode tipinde sonuc dondurur.
	...
	if(firsta==NULL){	//ilk eleman icin  (queuenin first u null yani eklenen,ilk eleman ise)
		...
		lasta->next=newNode;	//lastm ve lastm in nextini newNode yaptık
		lasta=newNode;
	}
...
struct movie* breadthfirst(struct movie* filmdizisi,int sayac){ // fonksiyon filmdizisi ve bu filmdizisinin boyutunu alarak bfs enine arama algoritmasi uygular.
	int i,j,isEmpty,h=0,bacon,count; //i film için, j aktör için, isempty bos/dolu kontrolu icin, h da hesaplanmisfilmdizisinin iterasyonu için
									//bacon, actorqueuedeki kbnumlarını kontrol etmek, count ise kac tane o aktordenoldugunu saymak icin
	struct movie* hesaplanmisfilmdizisi=(struct movie*)malloc(sayac*sizeof(struct movie));//kb numaralarine gore siraladigimiz queue elemanlarını kaybetmemek icin
	...
	actorqueue->data->kbnum=0;	//kevin baconın kevin bacon sayisi 0
	firsta=actorqueue;
	lasta=actorqueue;
	...
	while(isEmpty==0){ //dolu oldugu surece:
			...
			if(filmdizisi[i].kbnum==SONSUZ){	//tum film dizisi sonsuzdu basta
				...
				while(filmdizisi[i].actor[j].firstname[0]!=0){	//filmdizisinin actoru oldugu surece(actorun adinin ilk harfine bakiyoruz)
					if(strcmp(actorqueue->data->firstname,filmdizisi[i].actor[j].firstname)==0 && 	
						strcmp(actorqueue->data->lastname,filmdizisi[i].actor[j].lastname)==0){ //filmdizisindeki aktorun adı/soyadi queuedaki ada/soyada esit mi
		...
		while(iter!=NULL){	//queue bosalmadigi surece
			...
			while(iter->data->actor[j].firstname[0]!=0){
				if(strcmp(iter->data->actor[j].firstname,actorqueue->data->firstname)!=0 || 
					strcmp(iter->data->actor[j].lastname,actorqueue->data->lastname)!=0){//moviequeuedaki aktorun adı/soyadi queuedaki ada/soyada esit mi
						iter->data->actor[j].kbnum=iter->data->kbnum;	//queuedeki aktorun kbnumarası moviequeuedaki filmin kbsine esit
		...
		//esit degilse count ı artıracagiz. yani kbnum'ı o kadar olandan count tane var demek.kbnum ı 2 olan film varsa 2 tane olacak count.
			printf("Kevin Bacon sayisi %d olan oyuncu sayisi : %d\n", bacon, count);	
			bacon++;	//bacon sayilari esitse 1 artirdik 
			count=0;	//count 'ı her seferinde 0lıyoruz siradaki kbnumlı actore temiz bir sekilde gecmek icin.
		}
		count++;
...
void print(char* rootf, char* rootl, struct movie* filmdizisi, int sayac){	//adı verilen bir aktörün Kevin Bacon sayısını bulan ve bağlantıyı yazdirma
		...
		while(i<sayac && rootm==NULL){	//i<sayac yani satir sayisi ve movie root null
			...
			while(filmdizisi[i].actor[j].firstname[0]!=0 && rootm==NULL){
				if(strcmp(rootf, filmdizisi[i].actor[j].firstname)==0 && strcmp(rootl, filmdizisi[i].actor[j].lastname)==0){
					if(filmdizisi[i].kbnum==SONSUZ){	//kbnum ı sonsuz ise actorun adı soyadı ve yanina da sonsuz yazilsin.
					...
					printf("%s %s :\n", rootm->name, rootm->year);	//ve movie nin adı ve yılı yazilir
				}
				j++;
			}
			i++;
...
int main() {
	...
	int i=0,j=0,k=0,sayac=0;//i yi film için, j yi aktör için ve k yı aktörün ad/soyadının harfleri için,sayac'ı satır yani film sayisini bulmak icin kullanıyoruz. 
	char ch,file_name[25];	//ch text dosyasindaki karakterlerin iterasyonu icin, file_name ise kullanıcıdan alınacak dosyanin adi
	FILE *datafile;
	...
	datafile = fopen(file_name, "r");	//Dosya okuma işlevi
    if (datafile == NULL)
    {
        printf("cannot open file...\n");
    	exit(EXIT_FAILURE);
...
//    		printf("%c", ch);
		...
		//printf("%s\n",filmdizisi[i].name);
		k=0;
    	while(ch!='/'){	// slash a gelene kadari yıl
//    		printf("%c", ch);
...
//	    		printf("%c", ch);
			...
			k=0;
    		while(ch!='/' && ch!=10){	//satir sonu degilse ve slasha esit degilse aktorlerin devamı var 
//	    		printf("%c", ch);
    		...
    		if(ch=='/'){	// slash ı almak istemedigimizden
